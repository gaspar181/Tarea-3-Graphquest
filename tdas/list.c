...
void* list_get(List *lista, int indice) {
    void *elemento = list_first(lista);
    int i = 0;
    while (elemento != NULL && i < indice) {
        elemento = list_next(lista);
        i++;
    }
    return elemento;
}

void list_popIndex(List *L, int indice) {
    if (L == NULL || indice < 0 || indice >= list_size(L)) return;
